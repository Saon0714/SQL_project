                            ...
                            -- ALL THE FOLLOWING QUESTIONS ARE BASED ON (USE IG_CLONE) DATEBASE --

-- QUESTION 1: We want to reward the user who has been around the longest, Find the 5 oldest users.
...
SELECT DAYNAME(CREATED_AT) DAYNAME, COUNT(DAYNAME(CREATED_AT)) AS NO_OF_DAYS FROM USERS
...
SELECT * FROM LIKES;
SELECT USERNAME,USERS.ID FROM USERS INNER JOIN
(SELECT  ID ,USER_ID FROM PHOTOS INNER JOIN  
(SELECT COUNT(USER_ID),PHOTO_ID FROM LIKES
GROUP BY PHOTO_ID 
ORDER BY COUNT(USER_ID) DESC LIMIT 1) S
ON PHOTOS.ID = S.PHOTO_ID ) SS
ON USERS.ID = SS.USER_ID;

-- QUESTION 5: The investors want to know how many times does the average user post.


SELECT SUM(CT)/(SELECT COUNT(DISTINCT USER_ID) USER_COUNT FROM PHOTOS) AS AVERAGE_POSTS FROM
(SELECT USER_ID,COUNT(ID) CT FROM PHOTOS
GROUP BY USER_ID ORDER BY COUNT(ID) DESC) S
 ;
...
SELECT * FROM TAGS;
SELECT  TAG_NAME,COUNT(TAG_NAME) FROM PHOTO_TAGS PT
INNER JOIN TAGS T
ON T.ID = PT.TAG_ID
GROUP BY TAG_NAME
...
SELECT S.USER_ID,USERNAME FROM 
